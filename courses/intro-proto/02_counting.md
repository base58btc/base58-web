...
- First, for each fruit, fill in the max number of bytes we'd need in `how_many_bytes`,
- Next, edit the codeblock to see if you can make the `how_many_bytes` pass.


???
id: how-many-bytes

Have `how_many_bytes` return a dict with the correct byte sizes for each of the fruits
